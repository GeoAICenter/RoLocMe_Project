...
def get_target(inputs):
    ...
    lambda_return = terminated.at[-2].get() * rewards.at[-2].get() + GAMMA * (1.0 - terminated.at[-2].get()) * last_q

    last_q = jnp.max(q_values.at[-2].get(), axis=-1)
    last_q = jnp.sum(last_q, axis=-1)

    def calc_n_step_rewards(next_q, transition_per_step):
        done, q_val, reward = transition_per_step
        target_bootstrap = done * reward + GAMMA * (1 - done) * next_q
        next_q = jnp.sum(jnp.max(q_val, axis=-1), axis=-1)
        return next_q, target_bootstrap
    ...
    def make_check_point():
        def callback(callback_inputs):
            ...
            eqx.tree_serialise_leaves(
                f"/home/Users/PQLocNet_No_TD_Learning/SOTA/"
                f"PQLOCNET_DENSE_REWARDS_4_ROBOTS_CLIP_BY_5_LR_5e4_50M_CLONE_10_NO_TD_SPARSE_{val_count_.item()}_"
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(comment="CLIPPING_4_AGENTS_CLIP_BY_5_LR_5e4_50M_DENSE_CLONE_10_NO_TD_SPARSE")
