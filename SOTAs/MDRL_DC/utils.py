...
def get_all_maps(
    ...
    for dpm in dpms_imgs:
        ...
        # cv2.waitKey(0)
        buildings_embeddings.append(np.load(f'{path_to_buildings_embeddings}/{dpm.split("_")[0]}.npy'))

    dpms_list = jnp.array(dpms_list)
    buildings_list = jnp.array(buildings_list)
    floodfill = jnp.array(floodfill)
    buildings_embeddings = jnp.array(buildings_embeddings)
    return dpms_list, buildings_list, floodfill, buildings_embeddings
