...
def get_action(
    ...
    joint_actions = dist.sample(seed=sample_key, sample_shape=(1,)).squeeze().astype(jnp.int32)
...
def validation_simulation(
    ...
    def rollout(carry):
        states, b_embeddings, terminations, success, total_times, total_actions = carry
        ...
        new_states, new_terminations, new_success, new_total_times, new_total_actions = jax.vmap(env.val_step)(
            joint_actions,
            states,
            terminations,
            success,
    ...
    _, _, _, total_success_simulations, total_time_steps, total_actions_executed = jax.lax.while_loop(
        stop_func,
        rollout,
        (
            initial_states,
    ...
    return total_success_simulations, total_time_steps, total_actions_executed


@eqx.filter_jit
def mini_traj_rollout(rollout_inputs, _, num_envs):
    actor_params, sampled_key, continuous_reset_env_keys_from_rollout, continuous_env_states_from_rollout = rollout_inputs
    actor = eqx.combine(actor_params, actor_static_params)
...
@eqx.filter_jit
def train_model(inputs, _, gamma, gamma_lambda, entropy_coef, clip_eps, vf_coef, num_envs, num_simulations_val):
    (
        sample_action_key,
    ...
    (
        ...
        partial(mini_traj_rollout, num_envs=num_envs),
        (
            actor_params,
            sample_action_key,
            continuous_reset_env_keys,
    ...
    def make_check_point(actor_dynamic_params__, total_val_rewards_, total_times_, total_actions_, val_count_):
        def callback(callback_inputs):
            saved_actor_params, sum_val_rewards, total_times__, total_actions__, val_count__ = callback_inputs
            ...
            writer.add_scalar("Localization_time", total_times__.item(), val_count__.item())
            writer.add_scalar("Actions", total_actions__.item(), val_count__.item())
            writer.flush()
            # eqx.tree_serialise_leaves(
            #     f"/home/Users/MDRL_SR_FINAL/SOTA/SOTA_DENSE_FINAL_1e8_16_AGENTS_CLONE_11_{val_count__.item()}_PROPOSAL_{sum_val_rewards.item()}_TIME_{total_times__.item()}_ACTIONS_{total_actions__.item()}.eqx",
            #     saved_actor)
            pBar.update(val_count__.item(), values=[("loss", sum_val_rewards.item())])
            return None
    ...
    def logging(total_val_rewards_, total_times_, total_actions_, val_count_):
        def log_callback(log_callback_inputs):
            sum_val_rewards, total_times__, total_actions__, val_count__ = log_callback_inputs
            writer.add_scalar("Success_rate", sum_val_rewards.item(), val_count__.item())
            writer.add_scalar("Localization_time", total_times__.item(), val_count__.item())
            writer.add_scalar("Actions", total_actions__.item(), val_count__.item())
            writer.flush()
...
if __name__ == "__main__":
    ...
    dpms_list_train, buildings_list_train, floodfill_list_train, building_embeddings_train = get_all_maps(
        ...
        path_to_buildings_embeddings='/home/Users/DQN_LOCNET/Five_antennas_per_env_100_envs/Train_buildings_embeddings',
    )
    ...
    dpms_list_val, buildings_list_val, floodfill_list_val, building_embeddings_val = get_all_maps(
        ...
        path_to_buildings_embeddings='/home/Users/DQN_LOCNET/Five_antennas_per_env_100_envs/Val_buildings_embeddings',
    )
    ...
    key_action_, val_sim_key_, actor_key, critic_key, *env_keys_ = jax.random.split(
        key,
        1 + 1 + 1 + 1 + NUM_ENVS
    )
    ...
    writer = SummaryWriter(comment="AGENT_4_Sparse_1e8_CLONE_9")
